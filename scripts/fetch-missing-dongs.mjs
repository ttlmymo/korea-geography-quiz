...
function targetList(draft) {
  ...
  for (const gu of draft.gu) {
    ...
    for (const dongName of gu.missing || []) {
      targets.push({ gu, dong: { dongName, matchType: "missing" } });
    }
  }
  const seen = new Set();
...
for (const { gu, dong: target } of targets) {
  ...
  if (await exists(rawPath)) {
    ...
    const q = encodeURIComponent(`${gu.guName} ${target.dongName}`);
    const search = await apiGet(`/api/articles/search?q=${q}&p=1&ps=10`);
    if (search.status !== 200) {
      unresolved.push({ guName: gu.guName, sgg: gu.sgg, dongName: target.dongName, reason: `search HTTP ${search.status}`, candidates: [], response: search.text });
      console.log(`UNRESOLVED\t${gu.guName}\t${target.dongName}\tsearch-${search.status}`);
      continue;
    }
    const candidates = (parseJson(search.text, `${gu.guName} ${target.dongName} 검색`).items || [])
      .filter((item) => item?.headword === target.dongName || String(item?.headword || "").endsWith(target.dongName));
    if (candidates.length !== 1) {
      unresolved.push({ guName: gu.guName, sgg: gu.sgg, dongName: target.dongName, reason: candidates.length ? "ambiguousCandidates" : "noCandidate", candidates: candidates.map(summarize) });
    ...
    if (detail.status !== 200) {
      unresolved.push({ guName: gu.guName, sgg: gu.sgg, dongName: target.dongName, reason: `detail HTTP ${detail.status}`, candidates: candidates.map(summarize), response: detail.text });
    ...
    if (!searchable.includes(gu.guName)) {
      unresolved.push({ guName: gu.guName, sgg: gu.sgg, dongName: target.dongName, reason: "guNameNotInBodyOrDefinition", candidates: candidates.map(summarize) });
  ...
  const dongDoc = {
    ...(existing || {}),
    dongName: target.dongName,
    bjdCode,
    matchType: "dongDoc",
    eid: article.eid || "",
    raw: String(article.body || ""),
...
for (const gu of draft.gu) {
  gu.unresolved = unresolved.filter((item) => item.sgg === gu.sgg);
}
